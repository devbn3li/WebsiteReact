        ...
        >
          <div className="flex max-[991px]:flex-col">
            <div className="relative group cursor-pointer" >
              <div onClick={toggleDropDown} className='px-4 py-2 hover:bg-[#e6e6e5] rounded-lg transition-all duration-[.45s] inline-block max-[991px]:hover:bg-transparent max-[991px]:px-0 max-[991px]:py-3'>Services</div>
              <div className={`absolute w-[250px]  bg-[#03C2AB]  py-2 px-4 rounded-3xl shadow-lg group-hover:block ${dropDown ? 'block' : 'hidden'}`}>
                {dropdownItems.map((item, index) => (
                  <Link key={index} to={item.path} onClick={()=>{setDropDown(false)}} className="block text-[#fff] duration-500 hover:text-gray-600 py-1">
                    {item.title}
                  </Link>
                ))}
              </div>
