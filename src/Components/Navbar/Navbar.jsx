        ...
        >
          <div className="flex max-[991px]:flex-col justify-center items-center">
            <div className="relative group cursor-pointer" >
              <div onClick={toggleServicesActive} className='px-4 py-2 hover:bg-[#e6e6e5] rounded-lg transition-all duration-[.45s] inline-block max-[991px]:hover:bg-transparent max-[991px]:px-0 max-[991px]:py-3'>Services</div>
              <div className={`w-[250px]  bg-[#03C2AB] overflow-hidden flex flex-col justify-center px-4 min-[991px]:shadow-lg max-[991px]:w-screen group-hover:h-[360px] transition-all min-[991px]:rounded-3xl min-[991px]:absolute ${isServicesOpen ? 'h-[360px]' : 'h-0'}`}>
                {services.map((item, index) => (
                  <Link key={index} to={item.path} onClick={() => {setServicesActive(false) }} className="block text-[#fff] duration-500 hover:text-gray-600 py-1">
                    {item.title}
                  </Link>
                ))}
              </div>
            ...
            <div className="group cursor-pointer" >
              <div className='px-4 py-2 hover:bg-[#e6e6e5] rounded-lg transition-all duration-[.45s] inline-block max-[991px]:hover:bg-transparent max-[991px]:px-0 max-[991px]:py-3'>Solutions</div>
              <div className={`text-white w-screen flex justify-center items-center group-hover:translate-y-0 hover:delay-0 delay-100 translate-y-20 left-1/2 -translate-x-1/2 top-full bg-[#02C2AB] transition-all px-4 box-border border-b-[#3e857d] hover:border-b-4 group-hover:h-[150px] h-0 overflow-hidden max-[991px]:flex-col max-[991px]:hover:h-auto max-[991px]:translate-x-0 min-[991px]:absolute gap-1`}>
                <Link to='/solution' className="h-[130px] max-[991px]:h-[200px] w-1/2 flex justify-center items-center hover:bg-white/10 rounded-xl flex-col gap-2 max-[991px]:w-full"><span className="text-2xl"><FaLeaf /></span><span>Solutions 1</span></Link>
                <Link to='/solution' className="h-[130px] w-1/2 flex justify-center items-center hover:bg-white/10 rounded-xl flex-col gap-2 max-[991px]:w-full"><span className="text-2xl"><IoEarthSharp /></span><span>Solutions 2</span></Link>
              </div>
            </div>
